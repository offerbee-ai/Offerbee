...
// The chooser (/app/add) routes here with `?notice=` when Plaid connect
// fails, so manual search is never a dead end.
function NoticeBanner() {
  const notice = useSearchParams().get("notice");
  if (!notice) return null;
  return <p className="mt-3 text-[13px] font-medium text-warning">{notice}</p>;
}
