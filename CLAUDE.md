...
- Convex-side env (set in the Convex dashboard, not files): `CLERK_JWT_ISSUER_DOMAIN`, optional `OPENAI_API_KEY`, and for billing `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_ID_MONTHLY`, `STRIPE_PRICE_ID_YEARLY`, `SITE_URL` (per deployment — test keys on dev/staging, live keys on prod). The 14-day trial runs from account creation (`billingCore.TRIAL_MS`); support can extend an account via the `users.trialEndsAt` override.
